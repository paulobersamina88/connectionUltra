...
def first_existing(df, candidates):
    ...
    for cand in candidates:
        if str(cand).strip().lower() in m:
            return m[str(cand).strip().lower()]
    return None
...
def infer_shapes_columns(df):
    return {
        ...
        "b": first_existing(df, ["B","b","Ht"]),
        "h": first_existing(df, ["H","h","d"]),
    }
...
def normalize_shapes_df(df):
    ...
    work = df.copy().rename(columns={v:k for k,v in mapping.items() if v is not None})
...
def load_shapes_database(uploaded_file):
    ...
    for sheet in excel.sheet_names:
        try:
            ...
            mapping = infer_shapes_columns(df)
            if mapping["shape"] is not None and any(mapping.get(k) is not None for k in ["d","bf","tw","tf","t"]):
                valid_df = df
                valid_sheet = sheet
                break
        except Exception:
...
def default_shapes_db():
    data = [
        ...
        ["HSS12X8X1/2","HSS",12.0,8.0,None,None,52.0,46.0,276.0,2.70,18.5,0.500,None,8.0,8.0],
        ["PIPE12STD","PIPE",12.75,None,None,None,34.0,31.0,196.0,2.75,11.9,0.406,12.75,12.75,None],
    ]
...
def compute_connection(inp, beam, col):
    ...
    basis_cap = strength_result(basis, method, 0.9, 1.67) if not pd.isna(basis) else np.nan
    results.append({"Check":"Moment basis","Demand":Mu,"Available":basis_cap,"Units":"kip-in","Status":"OK" if not pd.isna(basis_cap) and Mu <= basis_cap else "NG","Notes":"Mp for non-seismic, Mpr for seismic."})

    lam, lam_p, okc = beam_flange_compactness(beam, fyb)
    results.append({"Check":"Beam flange compactness","Demand":lam,"Available":lam_p,"Units":"-","Status":"OK" if okc else "NG","Notes":"Compactness screen."})
    ...
    if conn == "Welded flange + bolted web":
        ...
        results.append({"Check":"Flange weld strength","Demand":flange_force,"Available":weld_cap,"Units":"kip","Status":"OK" if flange_force <= weld_cap else "NG","Notes":"Simplified screening."})
        ...
        results.append({"Check":"Web bolt group shear","Demand":Vu,"Available":bolt_cap,"Units":"kip","Status":"OK" if Vu <= bolt_cap else "NG","Notes":"Simplified web shear check."})

    if conn == "Bolted end plate":
        bolt_nom = bolt_shear_nominal(inp["ep_tension_bolts"], inp["bolt_dia"], inp["bolt_fu"], inp["threads_excluded"])
        bolt_cap = strength_result(bolt_nom, method, 0.75, 2.0)
        results.append({"Check":"End-plate tension bolt group","Demand":flange_force,"Available":bolt_cap,"Units":"kip","Status":"OK" if flange_force <= bolt_cap else "NG","Notes":"Proxy for tension-side bolt force."})
        ep_nom = plate_flexural_nominal(inp["ep_width"], inp["ep_thk"], fyb)
        ep_cap = strength_result(ep_nom, method, 0.9, 1.67)
        results.append({"Check":"End-plate flexure","Demand":Mu,"Available":ep_cap,"Units":"kip-in","Status":"OK" if Mu <= ep_cap else "NG","Notes":"Screening only."})

    if conn == "WUF-W seismic":
        weld_len = max(float(beam.get("bf", 0) or 0), 1e-6)
        weld_nom = 2 * weld_strength_per_inch(inp["fexx"], inp["flange_weld_size"]) * weld_len
        weld_cap = strength_result(weld_nom, method, 0.75, 2.0)
        results.append({"Check":"WUF-W flange weld","Demand":flange_force,"Available":weld_cap,"Units":"kip","Status":"OK" if flange_force <= weld_cap else "NG","Notes":"Prequalified connection screening only."})
    ...
    if conn == "RBS seismic":
        ...
        mrbs_cap = strength_result(mrbs, method, 0.9, 1.67) if not pd.isna(mrbs) else np.nan
        results.append({"Check":"RBS reduced section moment","Demand":Mu,"Available":mrbs_cap,"Units":"kip-in","Status":"OK" if not pd.isna(mrbs_cap) and Mu <= mrbs_cap else "NG","Notes":"Simplified RBS reduction."})

    panel_demand = Mu / max(float(col.get("d", 0) or 0)/2 + inp["connection_ecc"], 1e-6)
    panel_nom = panel_zone_nominal(col, fyc)
    panel_cap = strength_result(panel_nom, method, 0.9, 1.5) if not pd.isna(panel_nom) else np.nan
    results.append({"Check":"Panel zone shear","Demand":panel_demand,"Available":panel_cap,"Units":"kip","Status":"OK" if not pd.isna(panel_cap) and panel_demand <= panel_cap else "NG","Notes":"Approximate panel zone screening."})
    ...
    if not pd.isna(col_m) and not pd.isna(beam_m):
        ...
        results.append({"Check":"Strong-column / weak-beam ratio","Demand":limit,"Available":ratio,"Units":"-","Status":"OK" if ratio >= limit else "NG","Notes":"Frame-level concept screen."})

    doubler = 0.0 if pd.isna(panel_cap) or panel_demand <= panel_cap else (panel_demand - panel_cap) / max(0.6 * fyc * float(col.get("d", 1) or 1), 1e-6)
    extras = {
...
if run:
    ...
    k3.metric("Max utilization", "-" if util.dropna().empty else f"{util.max():.2f}")
    ...
    st.download_button("Download Excel report", out, "steel_connection_studio_results.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
else:
